import { PostType } from './types';

export const posts: PostType[] = [
  {
    postTitle: 'Title 1',
    description:
      "There are many variations of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which don't look even slightly believable.",
    postLink: ' ',
    tools: ['Next.js', 'React', 'Node.js', 'Tailwind CSS', 'TypeScript'],
  },
  {
    postTitle: 'Title 2',
    description:
      "If you are going to use a passage of Lorem Ipsum, you need to be sure there isn't anything embarrassing hidden in the middle of text. All the Lorem Ipsum generators on the Internet tend to repeat predefined chunks as necessary",
    postLink: ' ',
    tools: ['Next.js', 'React', 'Node.js', 'Tailwind CSS', 'TypeScript'],
  },
  {
    postTitle: 'Title 3',
    description:
      'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Curabitur quis feugiat metus. Proin nec justo nec augue accumsan suscipit ac feugiat eros. Nulla felis quam, bibendum nec luctus vitae',
    postLink: ' ',
    tools: ['Next.js', 'React', 'Node.js', 'Tailwind CSS', 'TypeScript'],
  },
];
